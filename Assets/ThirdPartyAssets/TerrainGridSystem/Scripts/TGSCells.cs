...
namespace TGS {
    ...
    public partial class TerrainGridSystem : MonoBehaviour {
        ...
        /// </summary>
        public List<int> CellGetNeighbours(Cell cell, int maxSteps, int cellGroupMask = -1, int maxSearchCost = 0, CanCrossCheckType canCrossCheckType = CanCrossCheckType.Default, int maxResultsCount = int.MaxValue) {
            int cellIndex = CellGetIndex(cell);
        ...
        /// Get a list of cells which are nearer than a given distance in cell count
        /// </summary>
        public List<int> CellGetNeighbours(int cellIndex, int maxSteps, int cellGroupMask = -1, int maxSearchCost = 0, CanCrossCheckType canCrossCheckType = CanCrossCheckType.Default, int maxResultsCount = int.MaxValue) {
        ...
        /// Get a list of cells which are nearer than a given distance in cell count
        /// </summary>
        public int CellGetNeighbours(int cellIndex, int maxSteps, List<int> cellIndices, int cellGroupMask = -1, int maxSearchCost = 0, CanCrossCheckType canCrossCheckType = CanCrossCheckType.Default, int maxResultsCount = int.MaxValue, bool ignoreCellCosts = false) {
                ...
                if (cx >= 0 && cx < _cellColumnCount && cy >= 0 && cy < _cellRowCount) {
                    ...
                    if (cells[ci].iteration == cellIteration) {
                        ...
                        if (distanceFunction(cellIndex, ci) <= maxSteps) {
                            int stepsCount = FindPath(cellIndex, ci, tempListCells, out dummyCost, maxSearchCost, maxSteps, cellGroupMask, canCrossCheckType, ignoreCellCosts);
                            if (stepsCount > 0) {
        ...
        /// Get a list of cells which are nearer than a given distance in cell count
        /// </summary>
        public List<int> CellGetNeighboursWithinRange(int cellIndex, int minSteps, int maxSteps, int cellGroupMask = -1, int maxCost = -1) {
            if (cellIndex < 0 || cellIndex >= cells.Count)
                return null;
            Cell cell = cells[cellIndex];
            ...
            for (int x = cell.column - maxSteps; x <= cell.column + maxSteps; x++) {
                ...
                for (int y = cell.row - maxSteps; y <= cell.row + maxSteps; y++) {
                    ...
                    if (distanceFunction(cellIndex, ci) <= maxSteps) {
                        List<int> steps = FindPath(cellIndex, ci, maxCost, maxSteps, cellGroupMask);
                        if (steps != null) {
